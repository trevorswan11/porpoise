...
TEST_CASE("Indents over time") {
    ...
    SECTION("Empty indent") { REQUIRE(indent.current_branch() == ""); }

    SECTION("Single non-last") {
        const Indent::Guard g{indent, false};
        REQUIRE(indent.current_branch() == symbols::T_BRANCH);
    }

    SECTION("Single last") {
        const Indent::Guard g{indent, true};
        REQUIRE(indent.current_branch() == symbols::L_BRANCH);
    }
